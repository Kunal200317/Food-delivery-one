...
export async function POST(req) {    
    ...
    let verified = validatePaymentVerification({"order_id": body.razorpay_order_id, "payment_id": body.razorpay_payment_id },body.razorpay_signature,'IJM0eHFmjmdOLpxaT3e1S7Vi');
     ...
     if(verified){
        ...
        return NextResponse.redirect(`http://localhost:3000/order/${body.razorpay_order_id}?paymentdone=true`);
    }
    else{
        return NextResponse.redirect(`http://localhost:3000/cart?paymentdone=false`);
    }
}